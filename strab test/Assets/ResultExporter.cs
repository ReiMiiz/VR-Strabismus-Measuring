...
public class ResultExporter : MonoBehaviour
{
    ...
    public GameObject[] hideGameObject = new GameObject[10];
    ...
    private string uniqueFilename(int width, int height)
    {
        ...
        if (logname.text == null || logname.text.Length == 0)
            ...
            filename = string.Format("{0}/{1}.{2}", folder, logname.text, format.ToString().ToLower());
    ...
    void Update()
    {
        ...
        captureScreenshot |= Input.GetKeyDown(capKey);
        ...
        if (captureScreenshot || captureVideo)
        {
            ...
            for(int i = 0; i < hideGameObject.Length; i++)
                if (hideGameObject != null) hideGameObject[i].SetActive(false);
            ...
            new System.Threading.Thread(() =>
            {
                ...
                //debugtext.GetComponent<UnityEngine.UI.Text>().text = string.Format("Wrote screenshot {0} of size {1}", filename, fileData.Length);
            }).Start();

            // unhide optional game object if set
            for (int i = 0; i < hideGameObject.Length; i++)
                if (hideGameObject != null) hideGameObject[i].SetActive(true);
